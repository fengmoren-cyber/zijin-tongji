...
修复说明：
  ...
  v2.3.1 - 修复 --rules 参数未识别错误
           按钮行移至日志区上方，确保始终可见
  v2.3.2 - 余额字段改用 Optional[Decimal]，None 表示无余额列
  v2.3.3 - 新增平安银行 xlsx / 中国银行双语 xlsx 解析器
           修复海南鸿明 simple xlsx 余额读取
  v2.3.4 - 统一余额列别名查找（余额/账户余额/交易后余额/期末余额/结余）
           修复 parse_boc_rows(xls) 余额读取
  v2.3.5 - 修复"各账户余额"Sheet 合计行/总计行全部显示 0.00 的问题
           改用 Python 直接求和，不再依赖 WPS 不会刷新的 IFERROR 公式
"""
